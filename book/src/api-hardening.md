...
The startup log line tells you which posture you're in. Here it is from
a server started against the book's `book_demo` namespace with both a
key and one allowed origin (`--api-key demo-key --cors-origin https://example.org`):
...
A wrong key gets the identical response (the server doesn't distinguish
"missing" from "incorrect"), and the comparison is constant-time, so
response timing doesn't leak how many leading bytes were right. Present
it either way:
...
silently lock everyone out. It also can't make you think you're
protected when you aren't; check the `api_key=` field in the startup
log.
...
An odd or out-of-range `?cycle=` is a 400, not an empty `[]`, which is
the kind of answer that makes people think their data didn't load:
...
On that second one: `min_date`/`max_date` take ISO `YYYY-MM-DD`, not
the FEC's `MM/DD/YYYY`. The strong typing is the point. `min_amount` is
an `Option<Decimal>` and `min_date` is an `Option<NaiveDate>` in the
handler's parameter struct, so a bad value is rejected up front with a
message that says what was expected, instead of matching nothing or
...
(Illustrative: the book's server didn't fail during writing.) This is
deliberate. `sqlx::Error`'s `Display` output includes table and
